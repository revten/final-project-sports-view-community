	...
	<div class="board__container">
		<div class="board__top">
			<c:choose>
				<c:when test="${sessionScope.loginAccount ne null}">
					<a href="volleyball.reg.go"> 새글쓰기</a>
				</c:when>
				<c:otherwise>
					<a href="volleyball.reg.go" onclick="alert('로그인하세요')">새글쓰기</a>
				</c:otherwise>
			</c:choose>
		</div>
			...
			<thead>
				...
				<c:forEach var="p" items="${posts }">
					<tr>
						...
						<td><a
							href="volleyball.detail.go?post_id=${p.post_id }&post_member=${p.post_member}">${p.post_title }
		...
		<div class="board__bot">
			<div class="board__bot-search">
				<form action="volleyball.search.do" name="boardSearchForm"
			...
			<div class="board__bot-paging">
				<c:if test="${curPage != 1}">
					<span><a href="volleyball.page.change?pg=1">[맨처음]</a></span>
					<span><a href="volleyball.page.change?pg=${curPage - 1}">
							◀이전 </a></span>
				</c:if>
				<c:forEach var="i" begin="1" end="${pageCount}">
					<a href="volleyball.page.change?pg=${i}"> [${i}] </a>
				</c:forEach>
				<c:if test="${curPage != pageCount}">
					<span> <a href="volleyball.page.change?pg=${curPage + 1}">
							다음▶ </a></span>
					<span><a href="volleyball.page.change?pg=${pageCount}">[맨끝]</a></span>
				</c:if>
			</div>
		</div>
