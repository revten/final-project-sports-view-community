					...
					<c:choose>
						...
						<c:otherwise>
							<button class="post__bot-btn"
								onclick="location.href='volleyball.update.go?post_id=${post.post_id}&post_member=${post.post_member}'">수정</button>
							<button class="post__bot-btn"
		...
		<table class="reply__table">
			<tr>
				<td><c:forEach var="pr" items="${post.replies }">
						...
						<form action="volleyballReply.reg.do"
...
function deleteReply(n, pid, pm) {
	...
	if (ok) {
		location.href = "volleyballReply.delete.do?reply_id=" + n + "&post_id=" + pid + "&post_member=" + pm;
	}
}
