<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
	<br>
	<br>
	<br>
	<br>
	<br>
	<br>
	<div id="stadium__container">
	<div id="stadium__area">
		<div id="stadium__img">
			<a style="cursor: pointer" onclick="baseballStadiumGo('ssg')"> <img
				style="width: 100px; height: 100px; cursor: pointer"
				src=resources/files/stadium/baseball/ssg_landers.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('kiwoom')">
				<img style="width: 100px; height: 100px; cursor: pointer"
				src=resources/files/stadium/baseball/kiwoom.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('lg')"> <img
				style="width: 100px; height: 100px; cursor: pointer"
				src=resources/files/stadium/baseball/lg_twins.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('kt')"> <img
				style="width: 100px; height: 100px; cursor: pointer"
				src=resources/files/stadium/baseball/kt_wiz.svg>
			</a>
		</div>
	</div>
	<div id="stadium__area">
		<div id="stadium__img">
			<a style="cursor: pointer" onclick="baseballStadiumGo('kia')"> <img
				style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/kia_tigers.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('nc')"> <img
				style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/nc_dinos.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('samsung')">
				<img style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/samsung_lions.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('lotte')">
				<img style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/lotte_giants.svg>
			</a>
		</div>
	</div>
	<div id="stadium__area">
		<div id="stadium__img">
			<a style="cursor: pointer" onclick="baseballStadiumGo('doosan')">
				<img style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/doosan_bears.svg>
			</a> <a style="cursor: pointer" onclick="baseballStadiumGo('hanwha')">
				<img style="width: 100px; height: 100px;"
				src=resources/files/stadium/baseball/hanwha_eagles.svg>
			</a>
		</div>
	</div>
	</div>
</body>
</html>