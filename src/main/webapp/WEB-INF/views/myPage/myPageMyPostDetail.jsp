...
<div style="border:1px solid;width:70%;align:center;">
		...
		<div>
					...
					<c:choose>
							<c:when test="${sessionScope.loginAccount.member_id eq MyPost.member_id }">
								<c:choose>
									...
									<c:otherwise>
										<button onclick="location.href='myPost.update.go?mypost_no=${MyPost.post_no}'">수정</button>
										<button onclick="location.href='myPost.delete.do?mypost_no=${MyPost.post_no}'">삭제</button>
									</c:otherwise>
								</c:choose>
							</c:when>
						</c:choose>
				...
				<input type="hidden" name="mypost_no" value="${MyPost.post_no }">
			...
			<c:forEach var="mpc" items="${MyPost.post_comments }">
				<tr>
					<%-- <td>${myposts.ac_nick}</td> --%>
					 <td>${mpc.post_comment_content}</td>
					<td>${mpc.post_comment_date}</td> 
					<td><c:choose>
